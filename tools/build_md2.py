...
w()
w('> **神器的持有限制**：每件神器只能合成一次，所以預設每件只會有 1 個。')
w('> 例外是鐵匠 —— 鐵匠可以複製神器，但整場只能用一次，而且只能挑其中一件，')
w('> 所以一套配裝裡「最多有一件神器出現兩次」，其餘神器仍然各一件。')
w('> （此規則由玩家提供，地圖資料未記錄。）')
w()
w('符號：`+` 材料相加、`=` 合成結果、`→` 完成任務後變成、`←` 對左邊的裝備使用右邊的卷軸。')
w()
w('> 另有兩條配方不在這張表裡（xlsx 沒收錄），只列在第九節的個別裝備上：')
w('> **黃道十二宮** ＝ 潛能覺醒 ＋ 覺醒卷軸（玩家提供，Wiki 寫錯）；')
w('> **寒冰之球** ＝ 熔岩球 ＋ 石英法杖（玩家提供，與簡體版更新情報一致）。')
w()
w('> 另有一條配方已依新版更正：**矛與盾**的第三個素材由「石英法杖」改為「翡翠吊墜」')
w('>（玩家提供，與簡體版更新情報「矛盾体不再需要石英法杖，改用翡翠吊墜」一致）。')
w()
w('| # | 合成路線 | 產物 | 產物屬性 |')
w('|---:|---|---|---|')
