...
PIPE = [
    ...
    # 假單位中繼要排在召喚物前面：兩者都用 CreateUnit，但結論正好相反。
    # 弩砲手曾被標成「召喚物 -> 不繼承裝備」，實際上它的砲彈是中繼，
    # 傷害整個掛在英雄名下，DefCof 與穿透全吃。標錯會把配裝方向帶反。
    ('假單位中繼', re.compile(r'SaveReal\(hash,GetHandleId\([A-Za-z_0-9]+\),13,'),
     '技能建一隻空殼單位、把傷害存進它的 **key 13**，'
     '`Trig_HeroTakeDamage_Actions` 開頭會把攻擊者換回英雄再打一次 —— '
     '所以**傷害算英雄打的**：吃 DefCof、帶穿透、擊殺也算英雄的。'
     '**這跟召喚物完全相反，不要混為一談。**'
     '另外中繼重打的是存進去的固定值，所以引擎的濺射衰減會被繞過。'),
    ('召喚物', re.compile(r'CreateUnit\('),
     '召喚物**不繼承**主人的裝備觸發／狀態／傷害 +%，只吃主人技能公式裡'
     '明寫的屬性（通常是最大生命與技能強度）與原生光環。'
     '**注意**：若上面同時列出「假單位中繼」，那 `CreateUnit` 可能只是中繼用的'
     '空殼，不是真召喚物 —— 要看該單位有沒有自己的攻擊力與 timed life。'),
