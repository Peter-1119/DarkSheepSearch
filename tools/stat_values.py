...
STATS = [
    ...
    # --- 以下原本漏掉，導致配裝總計少算（都是無條件的百分比屬性） ---
    ('dmg',    r'наносимый урон',                  '造成傷害',   'Damage dealt',  'Наносимый урон', 1),
    ('dmgas',  r'наносимый урон от атак и умений', '攻擊與技能傷害', 'Attack & ability damage', 'Урон от атак и умений', 1),
    ('dmgns',  r'(?:наносимый )?урон (?:не от статусов|кроме статусов)',
                                                   '非狀態傷害', 'Non-status damage', 'Урон не от статусов', 1),
    ('defas',  r'защита от атак и умений',        '攻擊與技能減傷', 'Attack & ability protection', 'Защита от атак и умений', 1),
    ('takeas', r'получаемый урон (?:от атак и умений|кроме статусов)',
                                                   '受到的非狀態傷害', 'Non-status damage taken', 'Получаемый урон кроме статусов', 1),
    ('defns',  r'защита (?:не от статусов|от урона не от статусов)',
                                                   '非狀態減傷', 'Non-status protection', 'Защита не от статусов', 1),
    ('vamp',   r'(?:эффект )?вампиризма?',         '吸血',       'Lifesteal',     'Вампиризм', 1),
    ('cdmod',  r'перезарядка модификаторов',       '裝備技能冷卻', 'Modifier cooldown', 'Перезарядка мод.', 1),
    # 狀態傷害與防護
    ('dburn',  r'урон от поджога',                 '點燃傷害',   'Burn damage',   'Урон от поджога', 1),
    ('dbleed', r'урон от кровотечения',            '流血傷害',   'Bleed damage',  'Урон от кровотечения', 1),
    ('ddise',  r'урон от болезни',                 '疾病傷害',   'Disease damage', 'Урон от болезни', 1),
    ('dstat',  r'урон от статусов',                '狀態傷害',   'Status damage', 'Урон от статусов', 1),
    ('pburn',  r'защита от поджога',               '點燃防護',   'Burn protection', 'Защита от поджога', 1),
    ('pbleed', r'защита от кровотечения',          '流血防護',   'Bleed protection', 'Защита от кровотечения', 1),
    ('pdise',  r'защита от болезни',               '疾病防護',   'Disease protection', 'Защита от болезни', 1),
    ('pfrz',   r'защита от заморозки',             '冰凍防護',   'Freeze protection', 'Защита от заморозки', 1),
    # 其他
    ('revive', r'время воскрешения',               '復活時間',   'Revive time',   'Время воскрешения', 1),
    ('gold',   r'доход золота',                    '金幣收入',   'Gold income',   'Доход золота', 0),
    ('goldm',  r'ежеминутный доход золота',        '每分鐘金幣',  'Gold per minute', 'Ежеминутный доход', 0),
    ('goldk',  r'(?:золото за убийства|награда за убийство врагов)',
                                                   '擊殺金幣',   'Gold per kill', 'Золото за убийства', 0),
]
NUM = r'([-+]?\d+(?:\.\d+)?)\s*(%?)'
