...
主屬性 **敏捷** · 背包 **4 格** · 解鎖 0 · 定位 刺客 · **近戰**（攻擊距離 100）
...
`Trig_HeroSkills21_Actions`　war3map.j:53293
```jass
if Skill=='A08P' then
...
`Trig_HeroSkills21_Actions`　war3map.j:53309
```jass
...
`Trig_HeroSkills21_Actions`　war3map.j:53298
```jass
if GetUnitLevel(u2)<=1 then
set dmg=(GetUnitState((u2),UNIT_STATE_MAX_LIFE))*2.0
call UnitDamageTarget(u,u2,dmg,false,false,ATTACK_TYPE_CHAOS,DAMAGE_TYPE_UNIVERSAL,WEAPON_TYPE_WHOKNOWS)
...
`Trig_HeroGiveSkills21_Actions`　war3map.j:53380
```jass
...
`Trig_HeroAttack21_Actions`　war3map.j:53342
```jass
if GetUnitAbilityLevel(u,'A08L')==1 and UnitLifePercent(u)>=50.00 then
set dmg=dmg*1.50
endif
...
`Trig_HeroSkills21_Actions`　war3map.j:53267
```jass
if Skill=='A0YD' then
...
`Trig_HeroTakeDamage_Actions`　war3map.j:19833
```jass
