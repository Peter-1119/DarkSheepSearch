...
主屬性 **力量** · 背包 **4 格** · 解鎖 50000 · 定位 坦克/詛咒者 · 護甲類型 **強化（城牆）**（六種攻擊類型全部 ×1.00，比英雄護甲差）
...
`Trig_HeroSkills26_Actions`　war3map.j:54806
```jass
...
`Trig_HeroSkills26_Actions`　war3map.j:54831
```jass
if GetUnitAbilityLevel(u,'A0BE')>0 then
set t=CreateTimer()
call SaveUnitHandle(hash,GetHandleId(t),1,u)
...
`Trig_HeroSkills26_Actions`　war3map.j:54793
```jass
if Skill=='A0BB' then
...
`Trig_HeroSkills26_Actions`　war3map.j:54820
```jass
...
`Trig_HeroSkills26_Actions`　war3map.j:54841
```jass
...
`Trig_HeroTakeDamage_Actions`　war3map.j:19546
```jass
else
...
`Trig_HeroSkills26_Actions`　war3map.j:54764
```jass
if Skill=='A09W' then
...
`Trig_HeroSkills34_Actions`　war3map.j:57187
```jass
elseif Skill=='A09Z' then
call SetHeroStr(u,GetHeroStr(u,false)+8,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+2,true)
...
`Trig_HeroSkills21_Actions`　war3map.j:53276
```jass
...
`Trig_HeroSkills34_Actions`　war3map.j:57172
```jass
if Skill=='A01S' then
...
`Trig_CreateHeroDuo21_Actions`　war3map.j:43375
```jass
...
`Trig_CreateHero21_Actions`　war3map.j:43421
```jass
...
`Trig_HeroAttack21_Conditions`　war3map.j:53328
```jass
...
`Trig_HeroAttack21_Actions`　war3map.j:53331
```jass
...
`Trig_HeroGiveSkills21_Actions`　war3map.j:53377
```jass
