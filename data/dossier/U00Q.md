...
`Trig_HeroKills27_Actions`　war3map.j:55064
```jass
...
`Trig_HeroGiveSkills27_Actions`　war3map.j:55086
```jass
...
`Trig_HeroSkills27_Actions`　war3map.j:54965
```jass
...
`Trig_HeroSkills27_Actions`　war3map.j:54959
```jass
if Skill=='A0BY' then
set t=CreateTimer()
set Id=GetHandleId(t)
...
`Trig_HeroSkills27_Actions`　war3map.j:54949
```jass
elseif Skill=='A100' then
call SetHeroStr(u,GetHeroStr(u,false)+8,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+2,true)
...
女妖女王 `H049` —— 物件資料同本體，但 **JASS 有依單位型號分支**：`Trig_HeroPick_Actions`（45030）、`HeroQ27`（54868）、`Trig_HeroKills27_Actions`（55042）
