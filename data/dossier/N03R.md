...
主屬性 **智力** · 背包 **4 格** · 解鎖 50000 · 定位 法師/召喚師 · **遠程**（攻擊距離 550）
...
物件欄位（原型 `ANcl`）：`Ncl1 = 0.800000011920929`, `Ncl2 = 2`, `Ncl3 = 3`, `Ncl4 = 0.800000011920929`, `Ncl5 = 0`, `Ncl6 = [None, 'channel']`, `aare = 350.0`, `acdn = 18.0`, `alev = 5`, `amcs = [100, 112, 124, 136, 148]`, `aran = 800.0`, `atar = air,enemies,ground,neutral,organic,item,debris`
...
`Trig_HeroSkills34_Actions`　war3map.j:57201
```jass
if Skill=='A0GX' then
...
`Trig_HeroSkills34_Actions`　war3map.j:57223
```jass
...
物件欄位（原型 `ANcl`）：`Ncl1 = [1.0, None]`, `Ncl2 = [2, None, 1]`, `Ncl3 = [3, None, 1]`, `Ncl4 = [1.0, None]`, `Ncl5 = [0, None]`, `Ncl6 = ['chemicalrage', None, 'channel']`, `aare = 400.0`, `acdn = [120.0, None, 17.0]`, `alev = 1`, `amcs = [225, None, 80, 90, 100, 110, 120]`, `aran = [900.0, None, 700.0]`, `atar = ['air,ground,friend,neutral,self', None]`

呼叫共用引擎函式：`BurnUnit` —— 完整內容見 `_engine.md`。

實作：

`Trig_HeroSkills34_Actions`　war3map.j:57260
```jass
...
這幾段不是靠技能 ID 分派的，而是直接用單位型號 `N03R` 寫在共用函式的條件式裡
（常見於寫進傷害管線的被動）。照技能抽取抓不到，所以單獨列出來。

`Trig_DieHero_Actions`　war3map.j:46660
```jass
elseif GetUnitTypeId(u)=='N03R' or GetUnitTypeId(u)=='Nfir' then
set p=GetUnitLoc(u)
set u2=CreateUnitAtLoc(pl,'o01P',p,0)
call SaveLocationHandle(hash,Id,3,p)
call SaveUnitHandle(hash,Id,4,u2)
