...
`Trig_HeroSkills11_Actions`　war3map.j:49745
```jass
if Skill=='A039' then
...
`Trig_HeroSkills11_Actions`　war3map.j:49752
```jass
if GetUnitAbilityLevel(u,'B007')==1 then
set cof=0.15+0.05*I2R(GetUnitAbilityLevel(u,'A03D'))
call SetWidgetLife(u,GetWidgetLife(u)+dmg*cof)
...
`Trig_HeroSkills11_Actions`　war3map.j:49766
```jass
...
`Trig_HeroAttack11_Actions`　war3map.j:49860
```jass
if GetUnitAbilityLevel(u,'B007')==1 then
set cof=0.15+0.05*I2R(GetUnitAbilityLevel(u,'A03D'))
call SetWidgetLife(u,GetWidgetLife(u)+dmg*cof)
...
`Trig_HeroSkills11_Actions`　war3map.j:49773
```jass
...
`Hero11E`　war3map.j:49695
```jass
if GetUnitAbilityLevel(u,'A03C')==1 then
call SaveReal(hash,GetHandleId(u),4,LoadReal(hash,GetHandleId(u),4)-0.35)
```

`Trig_HeroSkills11_Actions`　war3map.j:49784
```jass
if GetUnitAbilityLevel(u,'A03C')==1 then
call SaveReal(hash,GetHandleId(u),4,LoadReal(hash,GetHandleId(u),4)+0.35)
endif
...
`Trig_HeroAttack11_Actions`　war3map.j:49855
```jass
...
`Hero11AttackCD`　war3map.j:49834
```jass
...
`Trig_HeroSkills11_Actions`　war3map.j:49727
```jass
if Skill=='A0I5' then
...
`Trig_HeroSkills11_Actions`　war3map.j:49787
```jass
...
`Trig_HeroSkills11_Actions`　war3map.j:49734
```jass
elseif Skill=='A0I7' then
call SetHeroStr(u,GetHeroStr(u,false)+3,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+4,true)
...
黑暗聖堂武士 `Ewrd` —— 物件資料同本體，但 **JASS 有依單位型號分支**：`Trig_i_Actions`（16729）、`Trig_HeroPick_Actions`（44803）、`Trig_HeroKills11_Conditions`（49805）、`Trig_HeroKills11_Actions`（49811）、`Trig_HeroAttack11_Conditions`（49832）
...
`Trig_HeroKills11_Conditions`　war3map.j:49804
```jass
...
`Trig_HeroKills11_Actions`　war3map.j:49807
```jass
...
`Trig_HeroAttack11_Conditions`　war3map.j:49831
```jass
