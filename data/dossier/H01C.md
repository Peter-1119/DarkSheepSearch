...
主屬性 **力量** · 背包 **6 格** · 解鎖 0 · 定位 砲擊/輔助 · **遠程**（攻擊距離 1000） · 護甲類型 **重甲**
...
這幾段不是靠技能 ID 分派的，而是直接用單位型號 `H01C` 寫在共用函式的條件式裡
（常見於寫進傷害管線的被動）。照技能抽取抓不到，所以單獨列出來。

`Trig_HeroLvlUp_Actions`　war3map.j:45490
```jass
if GetUnitTypeId(u)=='H01C' then
if GetUnitAbilityLevel(u,'A0QX')==1 then
if GetHeroLevel(u)==25 then
call SetUnitBaseDamage(u,GetUnitBaseDamage(u)+50)
call SaveReal(hash,u_Id,16,LoadReal(hash,u_Id,16)+25.)
else
call SetUnitBaseDamage(u,GetUnitBaseDamage(u)+10)
call SaveReal(hash,u_Id,16,LoadReal(hash,u_Id,16)+5.)
endif
else
call SetUnitBaseDamage(u,GetUnitBaseDamage(u)+15)
endif
```
