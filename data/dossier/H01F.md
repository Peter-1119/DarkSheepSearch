...
主屬性 **智力** · 背包 **6 格** · 解鎖 0 · 定位 醫者/輔助/法師 · **遠程**（攻擊距離 700）
...
`Trig_HeroSkills23_Actions`　war3map.j:53811
```jass
if Skill=='A0XP' then
...
`Trig_HeroCheckSkills23_Actions`　war3map.j:53871
```jass
...
`Trig_HeroSkills23_Actions`　war3map.j:53833
```jass
...
`Trig_HeroSkills23_Actions`　war3map.j:53746
```jass
...
`Trig_HeroSkills23_Actions`　war3map.j:53727
```jass
if Skill=='A0YP' then
...
`Trig_HeroSkills23_Actions`　war3map.j:53735
```jass
elseif Skill=='A0YR' then
call SetHeroStr(u,GetHeroStr(u,false)+3,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+2,true)
...
`Trig_CreateHero23_Actions`　war3map.j:44019
```jass
