...
`Trig_HeroSkills5_Actions`　war3map.j:47984
```jass
...
`Trig_HeroSkills5_Actions`　war3map.j:47999
```jass
...
`Trig_HeroAttacks5_Actions`　war3map.j:48053
```jass
...
`Trig_HeroSkills5_Actions`　war3map.j:47976
```jass
if Skill=='A033' then
set t=CreateTimer()
set Id=GetHandleId(t)
...
`Trig_HeroSkills5_Actions`　war3map.j:47958
```jass
if Skill=='A0U5' then
...
`Trig_HeroSkills5_Actions`　war3map.j:47966
```jass
elseif Skill=='A0U7' then
call SetHeroStr(u,GetHeroStr(u,false)+1,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+3,true)
...
海之獵手 `Etyr` —— 物件資料同本體，但 **JASS 有依單位型號分支**：`Trig_HeroPick_Actions`（44715）
...
`Trig_CreateHero5_Actions`　war3map.j:37435
```jass
...
`Trig_CreateHero5_Hard_Actions`　war3map.j:37960
```jass
