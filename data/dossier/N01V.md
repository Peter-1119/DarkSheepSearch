...
**縮放**：吃技能強度的技能 ['A07L', 'A081', 'A082'] ／ ◈ 吃裝備技能威力 無 ／ ⊕ 給裝備技能威力 無
...
物件欄位（原型 `Aroa`）：`Roa1 = [-0.15000000596046448, -0.20999999344348907, -0.27000001072883606, -0.33000001311302185, -0.38999998569488525]`, `Roa2 = [-3, -4, -5, -6, -7]`, `aare = [None, 500.0]`, `abuf = B00M`, `acdn = 25.0`, `adur = 30.0`, `ahdu = 15.0`, `aher = 1`, `alev = 5`, `amcs = [85, 93, 101, 109, 117]`, `atar = air,ground,enemy,neutral`
...
技能「求生」每缺少 1% 生命值，就多回復 1% 生命值。

技能「野性戰士」在每次獲得的效果結束後，有 5% 機率保留 1 點力量。累積獲得 100 點力量後，機率降低至 3%。指令「-i」會顯示已獲得的力量。
```
...
技能「震懾咆哮」的作用範圍提升 50%，施放時造成（125% 力量）點傷害。

技能「斬擊者」改為另一種效果——對敵方英雄造成的傷害提高 75%，受到英雄的傷害降低 25%。

擊殺敵方英雄可提升 5 點力量。每擊殺 3 名英雄，力量的累積量便減少 1 點，最低為 1 點。
```
...
`Trig_HeroSkills15_Actions`　war3map.j:51070
```jass
elseif Skill=='A0MN' then
call SetHeroStr(u,GetHeroStr(u,false)+5,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+1,true)
...
## 斬擊者 `A0MQ`　—　來自皮膚「邪惡聖誕老人」

俄文原名：Слешер

```
對敵方英雄造成的傷害提高 75%，受到英雄的傷害降低 25%。
```

物件欄位（原型 `ANde`）：`Nde2 = 1.0`, `Nde3 = 1.0`, `Nde4 = 1.0`, `atar = enemies`

實作：

`Trig_HeroTakeDamage_Actions`　war3map.j:19835
```jass
...
物件欄位（原型 `Aroa`）：`Roa1 = [-0.15000000596046448, -0.20999999344348907, -0.27000001072883606, -0.33000001311302185, -0.38999998569488525]`, `Roa2 = [-3, -4, -5, -6, -7]`, `aare = 750.0`, `abuf = B00M`, `acdn = 25.0`, `adur = 30.0`, `ahdu = 15.0`, `aher = 1`, `alev = 5`, `amcs = [85, 93, 101, 109, 117]`, `atar = air,ground,enemy,neutral`
...
## 聖誕之力 `A0XC`　—　來自皮膚「邪惡聖誕老人」

俄文原名：Сила Рождества

```
Зелья действуют на героя на 30% эффективнее.

Умение "Выживание" восстанавливает на 1% больше здоровья за каждый недостающий 1% здоровья. Умение "Дикий воин" имеет 5% шанс сохранить 1 ед. силы после истечения каждого полученного эффекта. После получения 100 ед. силы шанс снижается до 3%. Команда "-i" отобразит полученную силу.

Область действия умения "Устрашающий рев" увеличена на 50%, также данное умение при применении наносит урон в размере (125% сила) ед. Умение "Слешер" приобретает другой эффект - увеличивает наносимый вражеским героям урон на 75% и уменьшает получаемый урон от героев на 25%. Убийства вражеских героев повышают силу на 5 ед. Накопление силы уменьшается на 1 ед. за каждые 3 уби
```
...
`Trig_HeroKills15_Conditions`　war3map.j:51119
```jass
function Trig_HeroKills15_Conditions takes nothing returns boolean
return GetUnitTypeId(GetKillingUnit())=='N01V' or GetUnitTypeId(GetKillingUnit())=='Hpb2'
endfunction
```

`Trig_HeroItems15_Actions`　war3map.j:51220
```jass
...
call SetUnitState(u,UNIT_STATE_MANA,mana+18)
elseif Id=='I00T' then
call SetUnitState(u,UNIT_STATE_LIFE,life+75)
call SetUnitState(u,UNIT_STATE_MANA,mana+30)
elseif Id=='pams' then
call SetUnitState(u,UNIT_STATE_LIFE,life+120)
call SetUnitState(u,UNIT_STATE_MANA,mana+60)
elseif Id=='I07I' then
