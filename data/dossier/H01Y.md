...
主屬性 **智力** · 背包 **6 格** · 解鎖 4000000 · 定位 法師/詛咒者 · **遠程**（攻擊距離 600）
...
`HeroQ51_conditions`　war3map.j:62698
```jass
...
`Trig_HeroQ51_Actions`　war3map.j:62762
```jass
...
`HeroW51_conditions`　war3map.j:62827
```jass
...
`Trig_HeroW51_Stop_Conditions`　war3map.j:63002
```jass
...
`HeroE51_conditions`　war3map.j:63027
```jass
...
`HeroR51_conditions`　war3map.j:63089
```jass
...
`HeroD51_var1_conditions`　war3map.j:63211
```jass
...
`HeroD51_var2_conditions`　war3map.j:63344
```jass
...
`HeroD51_var3_conditions`　war3map.j:63416
```jass
...
## 額外技能 `A0E5`　—　來自皮膚「恐懼女主宰」

俄文原名：Дополнительные умения

```
Открывает список дополнительных умений героя.
```

物件欄位（原型 `Aspb`）：`aite = 0`, `spb1 = A0DS,A0PJ,A0PS`, `spb2 = 0`
...
`HeroQ51_EndDebuff`　war3map.j:62743
```jass
function HeroQ51_EndDebuff takes nothing returns nothing
local timer t=GetExpiredTimer()
local integer Id=GetHandleId(t)
...
`Trig_HeroW51_Actions`　war3map.j:62964
```jass
...
local real x=GetLocationX(p)
local real y=GetLocationY(p)
call SaveTimerHandle(hash,Id_u,20,t)
call SaveUnitHandle(hash,Id_t,1,u)
call SaveReal(hash,Id_t,1,x)
call SaveReal(hash,Id_t,2,y)
call SaveReal(hash,Id_t,3,1)
call TimerStart(t,0.,false,function HeroW51_Dmg)
if udg_ItemBonusDMG[n]>=1000 then
call SaveTimerHandle(hash,Id_u,21,t2)
call SaveUnitHandle(hash,Id_t2,1,u)
call SaveReal(hash,Id_t2,1,x)
call SaveReal(hash,Id_t2,2,y)
...
`Trig_HeroW51_Stop_Actions`　war3map.j:63005
```jass
function Trig_HeroW51_Stop_Actions takes nothing returns nothing
local unit u=GetSpellAbilityUnit()
local integer Id=GetHandleId(u)
...
`Trig_HeroE51_Actions`　war3map.j:63030
```jass
function Trig_HeroE51_Actions takes nothing returns nothing
local unit u=GetSpellAbilityUnit()
local unit u2
...
local player pl=GetOwningPlayer(u)
local integer count=LoadInteger(hash,Id,1)
local texttag text=LoadTextTagHandle(hash,Id,2)
...
local player pl=GetOwningPlayer(u)
local integer n=GetPlayerId(pl)+1
local timer t2=LoadTimerHandle(hash,Id,2)
local integer Id2=GetHandleId(t2)
local integer Id_u=GetHandleId(u)
local integer count=LoadInteger(hash,Id,1)
local texttag text
...
local integer int_bonus_all=LoadInteger(hash,Id_u,22)
local real spd_bonus=0
local real spd_bonus_all=LoadReal(hash,Id_u,23)
set count=count-1
if count>5 then
...
call SaveInteger(hash,Id_u,22,int_bonus_all)
call SaveReal(hash,Id_u,23,spd_bonus_all)
call SaveInteger(hash,Id,1,count)
call SaveInteger(hash,Id2,1,count)
elseif count==0 then
set text=LoadTextTagHandle(hash,Id2,2)
call DestroyTextTag(text)
call FlushChildHashtable(hash,Id)
...
call SaveInteger(hash,GetHandleId(pl),13,LoadInteger(hash,GetHandleId(pl),13)-1)
...
`Trig_HeroR51_Actions`　war3map.j:63171
```jass
...
call SaveTextTagHandle(hash,Id2,2,text)
call SaveTimerHandle(hash,Id,2,t2)
call SaveInteger(hash,Id,1,11)
call SaveInteger(hash,Id2,1,11)
call SaveTimerHandle(hash,Id_u,22,t)
call SaveTimerHandle(hash,Id_u,23,t2)
call SaveInteger(hash,GetHandleId(pl),13,LoadInteger(hash,GetHandleId(pl),13)+1)
...
`Trig_HeroD51_var1_Actions`　war3map.j:63275
```jass
...
set u2=CreateUnit(pl,'o028',ux,uy,180)
call SetUnitX(u2,ux)
call SetUnitY(u2,uy)
call UnitApplyTimedLife(u2,'BTLF',lifetime)
call SetUnitAnimation(u2,"birth")
set ux=PolarX(x,190,180)
set uy=PolarY(y,190,180)
set u2=CreateUnit(pl,'o028',ux,uy,0)
call SetUnitX(u2,ux)
call SetUnitY(u2,uy)
call UnitApplyTimedLife(u2,'BTLF',lifetime)
call SetUnitAnimation(u2,"birth")
...
set u2=CreateUnit(pl,'o028',ux,uy,90)
call SetUnitX(u2,ux)
call SetUnitY(u2,uy)
call UnitApplyTimedLife(u2,'BTLF',lifetime)
call SetUnitAnimation(u2,"birth")
...
set u2=CreateUnit(pl,'o028',ux,uy,270)
call SetUnitX(u2,ux)
call SetUnitY(u2,uy)
call UnitApplyTimedLife(u2,'BTLF',lifetime)
call SetUnitAnimation(u2,"birth")
...
if lifetime>10 then
call SaveInteger(hash,Id,1,6)
else
call SaveInteger(hash,Id,1,5)
endif
endif
set u=null
set u2=null
...
`HeroD51_var2_EndBuff`　war3map.j:63347
```jass
...
`Trig_HeroD51_var2_Actions`　war3map.j:63372
```jass
...
call SetHeroInt(u,int+bonus,true)
call SetHeroInt(u2,int2+bonus2,true)
call UnitAddAbility(u,'A0PK')
call UnitAddAbility(u2,'A0PK')
else
set bonus=int2/3
call SetHeroInt(u,int+bonus,true)
call SetHeroInt(u2,int2-bonus,true)
...
call SaveInteger(hash,Id,2,bonus2)
...
`HeroD51_var3_Wave`　war3map.j:63419
```jass
...
local unit u=LoadUnitHandle(hash,Id,1)
local unit u3
local group ug_check=LoadGroupHandle(hash,Id,2)
local group ug=CreateGroup()
local player pl=GetOwningPlayer(u)
local real x2
local real y2
local real range=LoadReal(hash,Id,5)
local real L=0.00
set range=range+100
call SaveReal(hash,Id,5,range)
if range<=400 then
loop
set L=L+25
exitwhen L>75
...
set u3=FirstOfGroup(ug)
exitwhen u3==null
if UnitAlive(u3)and IsUnitEnemy(u3,pl)and IsUnitInGroup(u3,ug_check)!=true then
call UnitDamageTarget(u,u3,dmg,false,false,ATTACK_TYPE_NORMAL,DAMAGE_TYPE_MAGIC,WEAPON_TYPE_WHOKNOWS)
...
`Trig_HeroD51_var3_Actions`　war3map.j:63505
```jass
function Trig_HeroD51_var3_Actions takes nothing returns nothing
local unit u=GetSpellAbilityUnit()
local unit u3
local integer Id_u=GetHandleId(u)
local timer t=LoadTimerHandle(hash,Id_u,22)
local timer t2=LoadTimerHandle(hash,Id_u,23)
local integer Id=GetHandleId(t)
local integer Id2=GetHandleId(t2)
local player pl=GetOwningPlayer(u)
local integer n=GetPlayerId(pl)+1
...
call SaveReal(hash,Id,2,angle)
call SaveReal(hash,Id,3,x)
call SaveReal(hash,Id,4,y)
call SaveUnitHandle(hash,Id,1,u)
call SaveGroupHandle(hash,Id,2,ug_check)
