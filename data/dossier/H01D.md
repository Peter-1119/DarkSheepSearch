...
`Trig_HeroSkills13_Actions`　war3map.j:50515
```jass
...
`Trig_HeroSkills13_Actions`　war3map.j:50502
```jass
...
`Trig_HeroSkills13_Actions`　war3map.j:50638
```jass
...
`Trig_HeroSkills13_Actions`　war3map.j:50648
```jass
if GetUnitAbilityLevel(u,'A03I')>=1 then
set t=CreateTimer()
...
`Trig_HeroSkills13_Actions`　war3map.j:50471
```jass
if Skill=='A05Z' then
...
`Trig_HeroSkills13_Actions`　war3map.j:50434
```jass
if Skill=='A0I3' then
...
`Trig_HeroSkills13_Actions`　war3map.j:50518
```jass
...
`Trig_HeroSkills13_Actions`　war3map.j:50444
```jass
elseif Skill=='A0IA' then
call SetHeroStr(u,GetHeroStr(u,false)+3,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+1,true)
...
`Trig_HeroSkills13_Actions`　war3map.j:50454
```jass
elseif Skill=='A0LE' then
call SetHeroStr(u,GetHeroStr(u,false)+5,true)
call SetHeroAgi(u,GetHeroAgi(u,false)+2,true)
...
血精靈統帥 `Hlgr` —— 物件資料同本體，但 **JASS 有依單位型號分支**：`Trig_HeroPick_Actions`（44822）
